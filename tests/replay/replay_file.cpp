...
ReplayIoStatus load_replay(const std::string& path, Replay& out, std::string& error) {
    ...
    while (std::getline(file, line)) {
        ...
        if (keyword == "version") {
            ...
            change.players[0] = mw::sim::InputFrame{static_cast<uint16_t>(p1)};
            change.players[1] = mw::sim::InputFrame{static_cast<uint16_t>(p2)};
...
ReplayIoStatus save_replay(const std::string& path, const Replay& replay) {
    ...
    write_line("#");
    write_line("# Generated by mw_replay_record. Do not hand-edit the hashes.");
...
mw::sim::InputPair input_at_frame(const Replay& replay, int32_t frame) {
    mw::sim::InputPair result{{mw::sim::InputFrame{0u}, mw::sim::InputFrame{0u}}};
